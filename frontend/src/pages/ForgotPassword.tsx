    ...
    <div className="min-h-screen bg-dark flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-1 text-sm">Te enviaremos un enlace de recuperación</p>
        </div>
        ...
        <div className="bg-dark-surface border border-dark-border rounded-2xl p-8 shadow-dark-lg">
          {sent ? (
            <div className="text-center space-y-4">
              ...
              <p className="text-slate-300 text-sm">Correo enviado. Revisa tu bandeja de entrada.</p>
              <p className="text-slate-500 text-xs">Si no lo encuentras, revisa la carpeta de spam.</p>
              <Link to="/login" className="block w-full text-center bg-ucips-gold text-ucips-navy py-3 rounded-xl font-bold hover:bg-ucips-gold-light transition">
                Volver al login
              </Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
              <div>
                ...
                <input {...register("email")} type="email" autoComplete="email" placeholder="correo@ucips.edu.mx"
                  className="w-full bg-dark-elevated border border-dark-border text-slate-100 placeholder-slate-600 px-4 py-3 rounded-xl focus:outline-none focus:border-ucips-gold/50 focus:ring-1 focus:ring-ucips-gold/30 transition" />
