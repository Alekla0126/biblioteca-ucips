    ...
    <div className="min-h-screen bg-dark">
      <div className="bg-dark-surface border-b border-dark-border py-6 px-4">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          <Link to="/" className="text-ink-faint hover:text-ink-soft transition text-sm">← Inicio</Link>
          <span className="text-dark-border">/</span>
          <h1 className="text-lg font-bold text-ink">Mi perfil</h1>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-5">
        ...
        <div className="bg-dark-surface border border-dark-border rounded-2xl p-6 flex items-center gap-5">
          ...
          <div className="min-w-0">
            <p className="text-lg font-bold text-ink truncate">{appUser.display_name ?? "Sin nombre"}</p>
        ...
        <div className="bg-dark-surface border border-dark-border rounded-2xl p-6">
          <h2 className="text-base font-bold text-ink mb-4">Editar información</h2>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-ink-soft mb-1.5">Nombre completo</label>
              <input {...register("display_name")}
                className="w-full bg-dark-elevated border border-dark-border text-ink px-4 py-3 rounded-xl focus:outline-none focus:border-ucips-gold/50 focus:ring-1 focus:ring-ucips-gold/30 transition" />
            ...
            <div>
              ...
              <input value={appUser.email} readOnly
                className="w-full bg-dark-elevated/50 border border-dark-border text-ink-faint px-4 py-3 rounded-xl cursor-not-allowed" />
            </div>
        ...
        {/* Quick actions */}
        <div className="bg-dark-surface border border-dark-border rounded-2xl p-6">
          <h2 className="text-base font-bold text-ink mb-4">Accesos rápidos</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {(appUser.role === "admin" || appUser.role === "profesor") && (
              <Link to="/subir"
                ...
                <div>
                  <p className="font-semibold text-ink-soft text-sm group-hover:text-ink">Subir libro</p>
                  <p className="text-xs text-ink-faint">Agregar recurso nuevo</p>
                </div>
              </Link>
            )}
            {appUser.role === "admin" && (
              <Link to="/admin"
                ...
                <div>
                  <p className="font-semibold text-ink-soft text-sm group-hover:text-ink">Aprobar profesores</p>
                  <p className="text-xs text-ink-faint">Panel de administración</p>
                </div>
              </Link>
            )}
            <Link to="/recursos"
              ...
              <div className="w-9 h-9 bg-dark-border/50 rounded-lg flex items-center justify-center flex-shrink-0">
              ...
              <div>
                <p className="font-semibold text-ink-soft text-sm group-hover:text-ink">Ver biblioteca</p>
                <p className="text-xs text-ink-faint">Explorar recursos</p>
              </div>
            </Link>
          </div>
        </div>
        ...
        {/* Account info */}
        <div className="bg-dark-surface border border-dark-border rounded-2xl p-6">
          <h2 className="text-base font-bold text-ink mb-4">Información de la cuenta</h2>
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              <dt className="text-ink-faint">Miembro desde</dt>
